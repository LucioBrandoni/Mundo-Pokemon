...
// Evento para ingresar nick con confirmación visual
formNick.addEventListener("submit", (e) => {
    e.preventDefault();
    const nuevoNick = nickInput.value.trim();
    if (nuevoNick) {
        showModalConfirm(`¿Estás seguro de que quieres llamarte ${nuevoNick}?`, (confirmado) => {
            if (confirmado) {
                nick = nuevoNick;
...
// Función al elegir Pokémon con confirmación visual
function seleccionarPokemon(index) {
    const elegido = starters[index];
    showModalConfirm(`¿Estás seguro de que quieres elegir a ${elegido.nombre}?`, (confirmado) => {
        if (confirmado) {
            starter = elegido;
